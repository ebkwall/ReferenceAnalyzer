...
namespace ReferenceAnalyzerTool
{
...

    {
        ...
        private EnvDTE.StatusBar _statusbar;
        ...
        private static ToolWindowPane _window;
        private static AnalyzerWindow _analyzerWindow;
        private static IVsWindowFrame _windowFrame;

        /// <summary>
        /// Initializes a new instance of the <see cref="ReferenceAnalyzer"/> class.
        /// Adds our command handlers for menu (commands must exist in the command table file)
        /// </summary>
        /// <param name="package">Owner package, not null.</param>
        /// <param name="commandService">Command service to add command to, not null.</param>
        private ReferenceAnalyzer(AsyncPackage package, OleMenuCommandService commandService)
        {
        ...
        /// </summary>
        public static ReferenceAnalyzer Instance
        {
            get;
            private set;
        }
        ...
        public static async Task InitializeAsync(AsyncPackage package)
        {
            ...
            Instance = new ReferenceAnalyzer(package, commandService);

            // Get the instance number 0 of this tool window. This window is single instance so this instance
            // is actually the only one.
            // The last flag is set to true so that if the tool window does not exists it will be created.
            _window = package.FindToolWindow(typeof(AnalyzerWindow), 0, true);
            if ((null == _window) || (null == _window.Frame))
            {
                throw new NotSupportedException("Cannot create tool window");
            }

            _analyzerWindow = (AnalyzerWindow)_window;
            _windowFrame = (IVsWindowFrame)_window.Frame;
        ...
        public void Execute(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                //IVsHierarchy vsSolution = dte.Solution.ToolWindows.SolutionExplorer;


                Array selectedItems = (Array)_solutionExplorer.SelectedItems;
                foreach (UIHierarchyItem item in selectedItems)
                {
                    // Obviously need to change this if more than one!
                    selectedItem = item;
                    break;
                }
                ...
                {
                    //selectedProject = GetSelectedProject(solution.Projects, selectedItem.Name);
                ...
                //string message = $"Selected Item is {selectedItem.Name} ChildItemsCount {selectedItem.Collection.Count}";
                //string title = "ReferenceAnalyzer";
                ...
                //_output.OutputString("\n\n\n--->>> Added Project Reference (ClassLibrary2) to Project (FolderProcessor1)!!!");
            }
            catch (Exception ex)
            {
            ...
            finally
            {
                ...
                if (_analyzerWindow == null) _analyzerWindow = new AnalyzerWindow();

                // Make sure it has a reference to this class
                _analyzerWindow.Analyzer = this;
                ...
                //form.ProjectReferences = _dicSolutionProjectReferences.Keys.ToList();
                //form.FileReferences = _dicSolutionFileReferences;
                //form.RootNode = selectedItem.Name;
                //form.ProposedFixes = _proposedReferenceFixes;
        ...
        {
            var task = ServiceProvider.GetServiceAsync(typeof(IVsSolution));
            Assumes.Present(task);
        ...
        {
            ThreadHelper.ThrowIfNotOnUIThread();
            var task = ServiceProvider.GetServiceAsync(typeof(SVsThreadedWaitDialogFactory));
            Assumes.Present(task);
